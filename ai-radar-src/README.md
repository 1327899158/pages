...
添加后在 Actions 中手动运行 Update AI Radar and publish Pages，或等待下次计划任务。配置了密钥的服务会在采集后自动翻译新增英文标题，按标题复用历史译文；每个服务每轮最多提交 10,000 个新字符。该限制不是月度费用上限，建议在服务商处设置预算或配额。配置多个服务会分别产生用量。
